...
import {setSearchQuery, setList} from "../../../redux/actions";
...
const SearchField = () => {
    ...
    const searchHandler = async (e) => {
        if((e && e.key === 'Enter') || !e){
            try{
                const data = await request('http://localhost:5000/api/hentai/search', "POST", {searchTags: searchParams.searchTags, query: searchParams.searchQuery})
   ...
   <>
    <div className="header__search">
        <button className="header__search-btn" onClick={() => setActive(true)}>
        ...
        <input type="text"
               ...
               placeholder="Search for poison" className="header__search-input" />
    </div>
    <div className={`header__searchField ${active && 'header__searchField_active'}`}>
        <input type="text"
               disabled={loading}
               onKeyDown={e => searchHandler(e)}
               value={searchParams.searchQuery}
        ...
        <button className="header__searchField-search">
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
                <path d="M21.71,20.29,18,16.61A9,9,0,1,0,16.61,18l3.68,3.68a1,1,0,0,0,1.42,0A1,1,0,0,0,21.71,20.29ZM11,18a7,7,0,1,1,7-7A7,7,0,0,1,11,18Z" />
            </svg>
        </button>
        <button className="header__searchField-close" onClick={() => setActive(false)}>
